...
base_length=40;
base_leadout=11;
base_extra_depth=2; //zmeneno puvodne 0
	...
	mounting_holes=default_mounting_holes)
{
	difference ()
	{
		...
		{		
			...
			translate([wade_body_offset-4,0,0])
			cube([wade_block_width,25,wade_block_depth]);	
			...
			difference()
			{
				translate([-block_bevel_r+wade_body_offset-4,0,0])
				cube([block_bevel_r*2+wade_block_width,
					base_thickness/2+block_bevel_r,wade_block_depth+base_extra_depth]);				
				translate([-block_bevel_r+wade_body_offset-4,block_bevel_r+base_thickness/2])
				cylinder(r=block_bevel_r,h=wade_block_depth+base_extra_depth,$fn=60);
				translate([wade_block_width+block_bevel_r-5+wade_body_offset,
					block_bevel_r-5+base_thickness/2])
				...
				translate([wade_block_width+block_bevel_r-5+wade_body_offset,base_thickness/2])
				cube([2*(block_bevel_r-3),2*(block_bevel_r-3),wade_block_depth+base_extra_depth]);
				
			}
			// "Domecek" pro idler
			translate([wade_body_offset,elevation,0]) difference(){
				translate([-18,12.8,0]) cube([18,10,wade_block_depth]);
				translate([-12,19,1.6]) cube([12,27,25]);
				translate([-18,3,-0.1]) rotate([0,0,45]) cube([10,10,30]);
			}
			...
			translate([-base_leadout+12,-0.5+4,wade_block_depth+base_extra_depth])
			cube([20,3.4,layer_thickness*5]);
			
			translate([-base_leadout+12,-0.5+4 + 3.4/2,wade_block_depth+base_extra_depth])
			cylinder(r=3.4/2,h=layer_thickness*5,$fn=16);
			
			translate([-base_leadout+12+20,-0.5+4 + 3.4/2,wade_block_depth+base_extra_depth])
			cylinder(r=3.4/2,h=layer_thickness*5,$fn=16);	
		...
		translate([large_wheel_translation[0]-filament_feed_hole_offset + 13,8.5,wade_block_depth/2]) rotate([-90,0,0]) 
		  nut_insert_hole();
...
echo("bhmh", mounting_holes)
	...
	translate(large_wheel_translation)
	{
			...
			// Vyrez pro lozisko
			rotate([0,0,-angle]) translate([-30/2,0,625_height-10/2]) cube([30,hole_for_625,10],center=true);
			
			translate([0,0,23])
			b625(h=625_height+0.1);
		
			translate([-13,-1,625_height+8*layer_thickness])
			b625(h=wade_block_depth);
		
			// Zkraceni dilu
			translate([-15,-22.3/2,-0.1]) cube([7,50,wade_block_depth+1]);
			rotate([0,0,-angle]) translate([-30/2,0,23+9/2]) cube([30,hole_for_625,9],center=true);
			
			translate([0,0,625_height+layer_thickness])	
			cylinder(r=10/2,h=wade_block_depth-(8+layer_thickness),$fn=64);
			
			rotate([0,0,-angle]) translate([-30/2,0,625_height+layer_thickness+10])
			cube([30,9.6,wade_block_depth-(8+layer_thickness)],center=true);

			
			// Prostor pro cervika na hnacim kolecku
			hull(){
			  translate([0,0,625_height+layer_thickness])	
			  cylinder(r=11/2,h=5,$fn=64);
			
			  translate([0,0,625_height+layer_thickness])	
			  cylinder(r=9.6/2,h=6,$fn=64);
			  
			  rotate([0,0,-angle]) translate([-30/2,0,625_height+layer_thickness+2.5])
			  cube([30,11,5],center=true);
			  
			  rotate([0,0,-angle]) translate([-30/2,0,625_height+layer_thickness+3])
			  cube([30,9.6,6],center=true);
			}
			...
			translate([-filament_feed_hole_offset,-40,wade_block_depth/2])
			rotate([90,0,0])
			rotate(360/16)
			cylinder(r=(teflon_tube?4.05/2:filament_feed_hole_d/2+0.1),h=wade_block_depth*3+elevation,center=true,$fn=16);	
	...
	difference(){
	  ...
	  union(){
	    translate([12,15,0]){
		  ...
		  for (idle=[-0.7,0.7])
	      {
		    translate([idler_mounting_hole_across*idle,16,-1])cylinder(r=2.2, h=24, $fn=20);
		    translate([idler_mounting_hole_across*idle,15,-1])cylinder(r=2.2, h=24, $fn=20);		  
	      }
		}
	  }
	}	
...
module groovemount_holes (extruder_recess_d,extruder_recess_h)
{	
	// Recess in base
	translate([0,0,-1])
