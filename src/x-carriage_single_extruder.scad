...
module x_carriage_base(){
 ...
 for(i=[-1,1])
 {
   translate([i*(part_width/2 - 9/2 + 2),x_rod_distance+12,7/2]) cylinder(r=9/2 -2,h=7,$fn=32,center=true);
   translate([i*(part_width/2 - 9/2 + 2),-12-v_corr,7/2]) cylinder(r=9/2 -2,h=7,$fn=32,center=true);
 }
 translate([0,-12-v_corr,7/2]) cube([part_width-9 + 4,9 - 4,7],center=true);
 translate([0,x_rod_distance+12,7/2]) cube([part_width-9 + 4,9 - 4,7],center=true); 
}
...
module x_carriage_holes()
{
  ...
  {
	translate([10,0,12]) rotate([0,0,-90]) nut_hole();
  ...
  for(i=[-1:1])
  {     
    translate([-i*(part_width/2 - 9 - 4.5),-12 - 1 - v_corr,8/2]) rotate([0,0,0]) zip_paska(4);
    translate([-i*(part_width/2 - 9 - 4.5),-19.5 - v_corr,8/2]) rotate([0,0,0]) cylinder(r=7,h=30,$fn=32,center=true);
  } 
 
  translate([0,x_rod_distance+12 + 6.5,9]) rotate([-40,0,0]) cube([part_width,9,7],center=true);
}
