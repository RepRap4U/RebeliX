...
echo("bhmh", mounting_holes)
	...
	translate([-24.5+64+4,-0.5,3]) {
		translate([-50,0,0]) {
			translate([0,0,(wade_block_depth+base_extra_depth)/2+11+layer_thickness]) cylinder(r=m3_diameter/2, h=wade_block_depth+base_extra_depth, center=true);
			translate([0, 0, 3.5]) cylinder(r=m3_washer_diameter/2, h=15.1, center=true);
		}
		
		translate([-20,0,0]) {
			translate([0,0,(wade_block_depth+base_extra_depth)/2+11+layer_thickness]) cylinder(r=m3_diameter/2, h=wade_block_depth+0.2+base_extra_depth, center=true);
			#translate([0, 0, 3.5]) cylinder(r=m3_washer_diameter/2, h=15.1, center=true);
		}
	}
	...
	translate(large_wheel_translation)
	{
			...
			translate([-wade_block_width+2,0,18])
			cube([wade_block_width,
			...
			translate([0,-hole_for_608/2,8]) rotate([0,180,0]) cube([30,hole_for_608,10]);
			...
			translate([-10,-22.3/2,20]) cube(10,10,9);
		
			translate([0,0,8+layer_thickness])
			cylinder(r=m8_clearance_hole/2,h=wade_block_depth-(8+2*layer_thickness)+2);	

			// Uprava_2 - Vyrez pro hnaci sroub
			translate([0,m8_clearance_hole/2,8+2*layer_thickness]) rotate([0,0,180])cube([15,m8_clearance_hole,5]);
			...
			translate([-filament_feed_hole_offset,-11.9,wade_block_depth/2])
			rotate([90,0,0])
			rotate(360/16)
			cylinder(r1=filament_feed_hole_d/2 + 2,r2=filament_feed_hole_d/2,h=8,center=true,$fn=18);	
	...
	for (idle=[-1,1])
	{
		...
		rotate([0,90,0])
		{
			...
			cylinder(r=m3_nut_diameter/2+0.12,h=idler_nut_thickness,$fn=6);	

			translate([0,10/2,wade_block_width-idler_nut_trap_depth+idler_nut_thickness/2])
			cube([m3_nut_diameter*cos(30)+0.2,10,idler_nut_thickness],center=true);
		}
	}
}
